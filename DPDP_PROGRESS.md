...
| Email notifications (see `docs/email-notifications.md`) need **no new consent gating** | Audited every EMAIL notification type implemented (order/payment/delivery/subscription lifecycle updates, OTP, welcome) and confirmed all are transactional — necessary to operate the account/order/subscription the customer already initiated, resting on the same contract/`ESSENTIAL_SERVICE` basis as the rest of the platform's transactional communication (§2 row 3), not a genuine marketing opt-in. No marketing/promotional email exists anywhere in this codebase to gate in the first place. If one is ever built, it must check `ConsentPurpose.MARKETING_COMMUNICATIONS` (already defined, already a genuine opt-in) before sending — documented as a future requirement, not implemented now since there's nothing to gate. |
