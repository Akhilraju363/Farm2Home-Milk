...
public class PaymentController {
    ...
    public ResponseEntity<PaymentResponse> initiate(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(paymentService.initiate(request, principal.userId()));
    }
    ...
    public ResponseEntity<PaymentResponse> findById(
            ...
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(paymentService.findById(id, principal.userId(), principal.isAdmin()));
    }

    @GetMapping("/order/{orderId}")
    @Operation(summary = "Get all payments for an order")
    public ResponseEntity<List<PaymentResponse>> findByOrder(
            @PathVariable UUID orderId,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(paymentService.findByOrderId(
    ...
    public ResponseEntity<PaymentResponse> refund(
            @PathVariable UUID id,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(paymentService.refund(id, principal.userId(), principal.isAdmin()));
    }
}
