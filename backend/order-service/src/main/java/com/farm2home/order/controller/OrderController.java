...
public class OrderController {
    ...
    @Operation(summary = "Get all orders for a subscription",
            description = "Every order generated by the daily subscription-order job for one subscription, "
                    + "most recent orderDate first. Customers can only see orders for a subscription that's "
                    + "actually theirs (a mismatched subscriptionId returns an empty page, not an error, so "
                    + "this never confirms whether an unknown/other id exists); admins see any.")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Orders retrieved (possibly empty if the subscription is new, isn't the caller's own, or has no matching orders)"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Missing or invalid bearer token", content = @io.swagger.v3.oas.annotations.media.Content)
    })
    public ResponseEntity<ApiResponse<Page<OrderResponse>>> getBySubscription(
        ...
        UUID customerId = principal.isAdmin() ? null : principal.userId();
        return ResponseEntity.ok(ApiResponse.success(
                "Subscription orders retrieved successfully", orderService.findBySubscription(subscriptionId, customerId, pageable)));
    }
    ...
    @Operation(summary = "Update order status",
               description = "Valid transitions: PENDING→ASSIGNED, ASSIGNED→OUT_FOR_DELIVERY, "
                           ...
                           + "This is the admin/staff-facing manual path; a delivery partner's own progress "
                           + "updates flow through delivery-service's assignment endpoint and are reflected "
                           + "here automatically, not via this endpoint.")
    @ApiResponses({
